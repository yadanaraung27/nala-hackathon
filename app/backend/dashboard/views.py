...
class SampleProtectedEndpointView(APIView):
    ...
    def get(self, request):
        with session_scope() as session:
            ...
            return Response(
                {"message": "This is a protected endpoint.", "user_id": user.id},
...
class KPIView(APIView):
    ...
    def get(self, request):
        with session_scope() as session:
            ...
            conversation_threads = db.get_student_conversations(
...
class AnalyticsContentTopicTypeView(APIView):
    ...
    def get(self, request, date_range):
        with session_scope() as session:
            ...
            content_topics_types = display.to_stacked_bar_chart(
                db.get_student_content_topics_types(
                    session, user_id=user.id, start_date=week_ago, end_date=today
                )
            )
            return Response(
    ...
    def get(self, request, date_range):
        with session_scope() as session:
            ...
            if date_range == "all":
                ...
                week_ago = today - timedelta(days=int(date_range))
            topics = display.to_dict(
                db.get_student_topics(
                    session, user_id=user.id, start_date=week_ago, end_date=today
                )
            )
...
class AnalyticsTopicTrendView(APIView):
    ...
    def get(self, request):
        with session_scope() as session:
            ...
            topics_trend = db.get_student_topics_trend(
                session, user_id=user.id, start_date=week_ago, end_date=today
            )
            return Response({"topics_trend": topics_trend}, status=status.HTTP_200_OK)
    ...
    def get(self, request, date_range):
        with session_scope() as session:
            ...
            if date_range == "all":
                ...
                week_ago = today - timedelta(days=int(date_range))
            conversations = display.to_dict(
                db.get_student_conversations_trend(
                    session, user_id=user.id, start_date=week_ago, end_date=today
                )
            )
            return Response({"data_result": conversations}, status=status.HTTP_200_OK)
