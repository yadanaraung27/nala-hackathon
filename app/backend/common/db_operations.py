...
def get_student_conversations(session, user_id: int):
    ...
    try:
        conversation_threads = (
            ...
            .filter(Conversation.created_by_id == user_id)
            .count()
        )
...
def get_unqiue_topics(session, user_id: int):
    ...
    try:
        unique_topics = (
            session.query(Conversation_Labeling.topic)
            .filter(Conversation_Labeling.user_id == user_id)
            .distinct()
            .count()
        )
    except Exception as e:
...
def get_total_time_spent(session, user_id: int):
    ...
    try:
        total_time_spent = (
            session.query(func.round(func.sum(Conversation_Labeling.duration)))
            .filter(Conversation_Labeling.user_id == user_id)
            .first()
        )
...
def get_student_content_topics_types(
    session, user_id: int, start_date: datetime, end_date: datetime
):
    """
    ...
    query_statement = (
        session.query(
            Conversation_Labeling.topic,
            Conversation_Labeling.type,
            func.count(Conversation_Labeling.message_id).label("count_messages"),
            func.sum(Conversation_Labeling.duration).label("sum_duration"),
        )
        .filter(Conversation_Labeling.user_id == user_id)
        .group_by(Conversation_Labeling.topic, Conversation_Labeling.type)
        .statement
    )
    if start_date is not None:
        query_statement = (
            ...
            .filter(
                Conversation_Labeling.user_id == user_id,
                Conversation_Labeling.created_date.between(
    ...
    df_content_topics_types = pd.read_sql_query(
        sql=query_statement, con=session.get_bind()
    )
...
def get_student_topics(session, user_id: int, start_date: datetime, end_date: datetime):
    ...
    query_statement = (
        ...
        .filter(Conversation_Labeling.user_id == user_id)
        .order_by(func.count(Conversation_Labeling.message_id).desc())
        .group_by(Conversation_Labeling.topic)
        .statement
    )
    if start_date is not None:
        query_statement = (
...
def get_student_topics_trend(session, user_id: int):
    ...
    query_statement = (
        session.query(
            Conversation_Labeling.topic,
            Conversation_Labeling.created_date,
            func.count(Conversation_Labeling.message_id).label("count_messages"),
        )
        .filter(Conversation_Labeling.user_id == user_id)
        .group_by(Conversation_Labeling.topic, Conversation_Labeling.created_date)
        .statement
    )
...
def get_student_conversations_trend(
    session, user_id: int, start_date: datetime, end_date: datetime
):
    """
    ...
    query_statement = (
        session.query(
            func.date(Conversation.created_date).label("x"),
            func.count(Conversation.id).label("y"),
        )
        .filter(Conversation.created_by_id == user_id)
        .group_by(func.date(Conversation.created_date))
        .order_by(func.date(Conversation.created_date))
        .statement
    )
    if start_date is not None:
        query_statement = (
            ...
            .filter(
                ...
                Conversation.created_date.between(
                    start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d")
                ),
            )
            .group_by(func.date(Conversation.created_date))
